...
#     for i in range(min(longueurChaineMax // 2, len(chaine))):
#         tableauEntree[2 * i] = ord(chaine[i]) / float(128.0)
#         tableauEntree[2 * i + 1] = i % 2
...
def Charger() :
    ...
    entrees.extend(zip(strings,[False]* len(strings)))
    shuffle(entrees)
    return entrees
